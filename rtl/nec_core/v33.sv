...
module V33(
    ...
    // Debug only: the PHYSICAL instruction address, PS*16 + PC. The offset
    // alone is not locatable in the ROM without the segment.
    output      [19:0]  dbg_pc,
    // Debug only: ES (the V-series calls it DS1). STOS/MOVS write through it,
    // so a wrong value here misdirects a whole `rep stosw` while the code path
    // still looks perfectly correct -- see HANDOFF #54.
    output      [15:0]  dbg_es,
...
// `automatic` so the locals below get a fresh lifetime per call. `cond` is
// assigned on every arm of an exhaustive 4-bit case so it was never actually
// wrong, but a static task lifetime is the same trap that produced the
// do_work bug and is not worth leaving armed.
task automatic handle_branch(input nec_decode_t dec);
    case(dec.opcode)
        OP_B_COND: begin
            bit cond = 0;
            case(dec.cond)
...
assign dbg_es = reg_ds1;
assign dbg_pc = {reg_ps, 4'd0} + {4'd0, cur_pc};
    ...
    if (reset) begin
        ...
        case(state)
            ...
            EXECUTE: begin
                ...
                if (dp_ready & ce_1 & |exec_delay & ~turbo) begin
                    ...
                    case(decoded.opcode)
                        ...
                        OP_STM: begin
                            bit do_work;
                            do_work = 1;   // MUST be a statement: a declaration initializer is
                                           // implicitly STATIC and runs only once (IEEE 1800 6.21)
            ...
            POP_WAIT: if (ce_1 & dp_ready) begin
                bit [15:0] list;
                int pop_idx;
                pop_idx = 0;   // same implicit-static trap as do_work: as a
                               // declaration initializer this runs once, so an
                               // all-zero pop_list would reuse the last index
